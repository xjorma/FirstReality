...
float FadeColourComponent(int t)
{
	float fProportion = 1.0;
...
int GetC1(int t)
{
	...
	int nResult = GetInitialK1(nPlasmaIndex) + (-3 * t);

	float fResult = float(nResult);
	fResult = fmod(fResult, 4096.0);
...
int GetC2(int t)
{
	...
	int nResult = GetInitialK2(nPlasmaIndex) + (-2 * t);
...
int GetC3(int t)
{
	...
	int nResult = GetInitialK3(nPlasmaIndex) + (1 * t);
...
int GetC4(int t)
{
	...
	int nResult = GetInitialK4(nPlasmaIndex) + (2 * t);
...
void Plasma_float(in float2 uv, in float iTime, out float3 color, out float alpha)
{
	...
	float fT = float(t);
	t = int(fmod(fT, 2323.0));

	// fPlasmaX is a float from 0 to 319,
	// fPlasmaY is a float from 0 to 399.
	float fPlasmaX = (uv.x) * 319.0;
	float fPlasmaY = (uv.y) * 399.0;
	...
	if ((fPlasmaY >= 0.0) && (fPlasmaY < 280.0))
	{
		...
		//int plasmaIndex = GetPerPlasmaTime(t);

		float c1 = float(GetC1(t));
		float c2 = float(GetC2(t));
		float c3 = float(GetC3(t));
		float c4 = float(GetC4(t));
		...
		float fade = FadeColourComponent(t);
		fRed *= fade;
		fGreen *= fade;
		fBlue *= fade;

		color = float3(fRed, fGreen, fBlue);
		float maxComponent = max(max(fRed, fGreen), fBlue);
		alpha = maxComponent * 0.6 + 0.2 * fade;
		return;
	}
	color = float3(0, 0, 0);
	alpha = 0;
